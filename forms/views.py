...
def OT_data(request):
    if request.method == 'POST':
        ...
        serializer = OTSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def physiotherapy_data(request):
    if request.method == 'POST':
        ...
        serializer = PhysiotherapySerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def dialysis_data(request):
    if request.method == 'POST':
        ...
        serializer = DialysisSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def firstfloor_rawdata(request):
    if request.method == 'POST':
        ...
        if selected_date and raw_data:
            ...
            return Response({"error": "selectedDate and raw_data are required"}, status=status.HTTP_400_BAD_REQUEST)
